...
pub async fn handle_request_with_scheme(frontend_scheme: &str, client_ip: IpAddr, req: Request<Body>) -> Result<Response<Body>> {
    ...
    let target = if let Some(sub) = &sub_route {
        ...
        if !is_websocket(&req) {
            ...
            let mut new_req = Request::builder().method(req.method()).uri(stripped_path).version(req.version()).body(req.into_body())?;
...
mod tests {
    ...
    fn test_extract_host_from_uri_authority() {
        let req = Request::builder().uri("http://example.com/path").body(Body::empty()).unwrap();
    ...
    fn test_extract_host_from_header() {
        let req = Request::builder().uri("/path").header("Host", "api.example.com").body(Body::empty()).unwrap();
    ...
    fn test_extract_host_from_header_with_port() {
        let req = Request::builder().uri("/path").header("Host", "api.example.com:8080").body(Body::empty()).unwrap();
    ...
    fn test_extract_host_priority_uri_over_header() {
        let req = Request::builder().uri("http://uri.example.com/path").header("Host", "header.example.com").body(Body::empty()).unwrap();
